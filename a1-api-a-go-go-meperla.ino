...
//const char* username = "meperla"; //GeoNames takes a username not an API key
...
void getGeoNames() {
  ...
  if (httpCode > 0) {
    if (httpCode == 200) {
      ...
      places.an = root["geonames"][0]["adminName1"].as<String>(); 
      places.la = root["geonames"][0]["lat"].as<String>(); 
